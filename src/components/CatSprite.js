  ...
  return (
    <>
      <svg
        ...
        width={size}
        height={(size * 100.04156036376953) / 95.17898101806641}
        viewBox="0.3210171699523926 0.3000000357627869 95.17898101806641 100.04156036376953"
        version="1.1"
        xmlSpace="preserve"
        id="spriteWrapper"
        style={{
          transform: `translate(${position.x}px, ${position.y}px) rotate(${turn}deg)`,
          transition: `transform ${duration}s ease-in-out`, // Duration and easing function
      ...
      {messageOpen && <SpeechBubble message={message} position={{ x: position.x, y: position.y - 50 }} />}
      {thinkOpen && <ThinkBubble message={thinkMessage} position={{ x: position.x, y: position.y - 100 }} />}
    </>
  );
}
